...
class AddTodo extends Component {
  ...
  handleChange(event) {
    ...
    if (e.which === 13) {
      this.props.actions.addTodo(value);  // =addTodo(value)
      }
    console.log('in todoreducer', this.props.TodoReducer);
    };
  ...
  render(){
    return (
      ...
      <h2 className="headTodo">My Todo Lists</h2>
        <input className="input" name="text" placeholder="Add Todo here"
           value={this.state.formValues["text"]} onChange={this.handleChange.bind(this)}/>

      <input className="input" name="desc" placeholder="Add description here"
      value={this.state.formValues["desc"]} onChange={this.handleChange.bind(this)}/>

<br/>
      <button className="btn btn-enter" type="submit" value="Submit"
